...
export function renderOllamaServerTest(result) {
  if (!result) return "";
  const models = (result.models || [])
    .map((item) => `<li><code>${escapeHTML(item.name)}</code> <span class="muted-text">${escapeHTML(item.digest)}</span></li>`)
    .join("");
  return `<div class="runtime-live"><div class="section-label">SERVER TEST (OBSERVATION)</div>${provenanceBadge(result.status)}${result.runtime_version != null ? `<div class="model-meta">runtime_version ${escapeHTML(result.runtime_version)}</div>` : ""}<p class="muted-text">A server test is observation, not model identity approval.</p>${models ? `<ul class="runtime-model-list">${models}</ul>` : ""}</div>`;
}
...
export function renderRuntimeServers(runtime, options = {}) {
  ...
  return servers
    .map((server) => {
      const id = server.id;
      return `<div class="runtime-item" data-runtime-server="${escapeHTML(id)}"><div class="model-card-top"><div class="model-title">${escapeHTML(id)}</div>${provenanceBadge(server.origin_source)}</div><div class="model-meta">${escapeHTML(server.origin)}</div><div class="model-meta">origin_source ${escapeHTML(server.origin_source)}</div><button type="button" class="ghost small" data-runtime-test-server="${escapeHTML(id)}" ${busy ? "disabled" : ""}>Test</button>${renderLiveServer(live[id])}${renderOllamaServerTest(tests[id])}</div>`;
    })
    .join("");
}
...
export function renderRuntimeWorkers(runtime, options = {}) {
  ...
  return workers
    .map((worker) => {
      ...
      return `<div class="runtime-item" data-runtime-worker="${escapeHTML(id)}"><div class="model-card-top"><div class="model-title">${escapeHTML(id)}</div></div><div class="model-meta">${escapeHTML(worker.kind)} · ${escapeHTML(worker.network_class)} · ${escapeHTML(worker.ollama_server_id)}</div><div class="model-meta">identity_approved: ${escapeHTML(worker.identity_approved)}</div>${renderSourcedValue("model_tag", worker.model_tag)}${renderSourcedValue("approved_model_digest", worker.approved_model_digest)}${renderSourcedValue("approved_runtime_version", worker.approved_runtime_version)}${renderSourcedValue("effective_context_tokens", worker.effective_context_tokens)}${renderSourcedValue("temperature", worker.temperature)}${renderSourcedValue("normalizer_id", worker.normalizer_id)}${renderSourcedValue("normalizer_version", worker.normalizer_version)}${correlation}${renderWorkerAttestation(attestations[id])}${renderRuntimeIdentityResult(identityResults[id])}<div class="runtime-actions"><button type="button" class="small" data-runtime-approve="${escapeHTML(id)}" ${disabled}>Approve live identity</button><button type="button" class="ghost small" data-runtime-replace-ask="${escapeHTML(id)}" ${disabled}>Approve new identity</button></div>${confirm}</div>`;
    })
    .join("");
}
