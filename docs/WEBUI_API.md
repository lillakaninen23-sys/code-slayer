...
| `POST /api/tailscale/enable` | `{ }` only. `tailscale serve --bg http://127.0.0.1:PORT`. Rejects non-loopback backends. Never Funnel. Trusted Host discovery is live (no restart required) |
