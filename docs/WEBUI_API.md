...
| `POST /api/tailscale/enable` | `{ }` only. If live Serve already proxies the configured loopback backend and Funnel is absent, persist `enabled=true` without re-running the CLI (adopting a matching live mapping is how config/live drift is aligned). Otherwise `tailscale serve --bg http://127.0.0.1:PORT`. Rejects non-loopback backends. Funnel present: 409, config unchanged. Never Funnel. Trusted Host discovery is live (no restart required) |
| `POST /api/tailscale/disable` | `{ }` only. `tailscale serve reset` unless live Serve is already `not_configured` (then persist `enabled=false` only) |
