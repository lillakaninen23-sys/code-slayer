...
**Accepted follow-up backlog:** the two P2 findings from the independent Phase-7 acceptance review remain OPEN and out of scope for WebUI Foundation 1. Their titles/reference IDs are not present in this checkout; retain both in the external acceptance record and add those references here when supplied. Neither is claimed fixed or removed by this slice.
