...
**Phase 8.1 — Repository Intelligence: implemented.** A deterministic, evidence-only repository-indexing foundation — bounded file inventory, project/language detection, discovered (never executed) test/lint/build commands, Python AST symbol extraction, an internal import graph, deterministic relevance ranking, and a bounded context-pack builder — durable across a restart and bound to an exact repository HEAD/working-tree identity so stale information is never served as current. See [`docs/REPOSITORY_INTELLIGENCE.md`](REPOSITORY_INTELLIGENCE.md) for the full evidence model, exclusion rules, limits, and refresh/invalidation semantics. Read-only: it never writes a repository file, never executes a discovered command, and grants no trust/policy/lease/cloud authority. Not yet implemented (later Phase 8 slices): planning, deterministic finalization gates, and independent review — this stage's remaining three items above.
