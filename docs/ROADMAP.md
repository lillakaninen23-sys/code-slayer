...
> **Current accepted baseline:** Phase 7 / **Local Worker Runtime = VERIFIED / ACCEPTED**, including the persistent LocalWorkerRunner and Phase 7.7a–7.7e hardening. Phase 6 leases/fencing remain the execution authority. WebUI Foundation 1 adds the thin HTTP/application view described in [`WEBUI_API.md`](WEBUI_API.md). **Phase 8 / Engineering Intelligence is IN PROGRESS**: Phase 8.1 (deterministic Repository Intelligence, content-safe as of 8.1a — see [`REPOSITORY_INTELLIGENCE.md`](REPOSITORY_INTELLIGENCE.md)) and Phase 8.2 (durable engineering planning — see [`ENGINEERING_PLANNING.md`](ENGINEERING_PLANNING.md)) are implemented; deterministic finalization gates and independent review are not yet started. No mutation trust, `AUTO`, or job-worktree-merge behavior was added by Phase 8.1 or 8.2 — Phase 8.2 is planning only and cannot execute anything it plans.
...
**Phase 8.1 — Repository Intelligence: implemented.** A deterministic, evidence-only repository-indexing foundation — bounded file inventory, project/language detection, discovered (never executed) test/lint/build commands, Python AST symbol extraction, an internal import graph, deterministic relevance ranking, and a bounded context-pack builder — durable across a restart and bound to an exact repository HEAD/working-tree identity so stale information is never served as current. Phase 8.1a hardened that identity to be content-safe (real content hashes for dirty/untracked evidence, never `stat()` metadata alone). See [`docs/REPOSITORY_INTELLIGENCE.md`](REPOSITORY_INTELLIGENCE.md) for the full evidence model, exclusion rules, limits, and refresh/invalidation semantics. Read-only: it never writes a repository file, never executes a discovered command, and grants no trust/policy/lease/cloud authority.

**Phase 8.2 — Engineering Planning: implemented.** Transforms an original engineering request plus authoritative Repository Intelligence into a structured, durable, evidence-backed `EngineeringPlanContent` — a provider-neutral `Planner` boundary, evidence validation that never lets a model's own existing-file/command/symbol claim become fact merely because it was asserted, full reuse of the existing hardened Question Gate for planner-raised ambiguity, and a plan state model (`DRAFT`/`NEEDS_INPUT`/`READY`/`SUPERSEDED`, with staleness always *computed* against live Repository Intelligence identity rather than durably stored). Phase 8.2b bounded production planner context (an explicit, centralized, planner-turn-specific budget, separate from Repository Intelligence's own indexing/query defaults) and formalized the `emit_engineering_plan` structured-output schema; Phase 8.2c diagnosed the live Devstral/Ollama planner runtime itself as not yet production-qualified for stable structured-tool compliance (independent of Code Slayer's own request payload, which was proven correct). Phase 8.2d converted planning execution into a server-owned, durable background job (`planning_jobs`, migration `0009`) — `POST /api/plans`/`.../replan` return `202` immediately, with HTTP client lifetime holding zero authority over whether a job continues, ownership/claim fencing mirroring `lease.manager`'s own pattern at a much smaller scope, and crash/restart recovery via the existing `lease.liveness` evidence. See [`docs/ENGINEERING_PLANNING.md`](ENGINEERING_PLANNING.md). Planning only: it never mutates a repository file, never executes a discovered command, never creates a checkpoint, never merges/promotes anything, and grants no `AUTO`/mutation trust — `READY` means a plan is internally consistent and evidence-backed, never that execution is authorized. Not yet implemented (later Phase 8 slices): deterministic finalization gates and independent review; a production-qualified local planner model/runtime.
